...
def cargar_csv():
    ...
    df['HomeTeam'] = df['HomeTeam'].replace(mapeo)
    df['AwayTeam'] = df['AwayTeam'].replace(mapeo)
...
def cargar_eventos(league, season):
    ...
    mapeo = {'Atletico Madrid':'Ath Madrid','Atlético Madrid':'Ath Madrid','Atletico de Madrid':'Ath Madrid','Athletic Bilbao':'Ath Bilbao','Athletic Club':'Ath Bilbao'}
    for col in ['HomeTeam','AwayTeam','equipo']:
        if col in df_g.columns:
            df_g[col] = df_g[col].astype(str).str.strip().replace(mapeo)
    ...
    for (ht, at, fecha), grupo in df_g.groupby(['HomeTeam','AwayTeam','Date']):
        ...
        eventos.append({"home_team": ht, "away_team": at, "date": fecha, "events": evs})
    return eventos
     

def buscar_goles_partido(row, eventos_list, min_min=0, max_min=120, parte="Todo", equipo_filtro=None):
    if pd.isna(row['Date']):
        return ""
    ...
    ganador = None
    if hg > ag: ganador = str(row['HomeTeam']).strip().lower()
    elif ag > hg: ganador = str(row['AwayTeam']).strip().lower()
    ...
    for m in eventos_list:
        ...
        if ht == ht_row and at == at_row and fecha == fecha_row:
            ...
            for ev in m.get('events', []):
                ...
                if equipo_filtro and equipo_filtro != "Ninguno" and team != equipo_filtro.strip().lower():
                    continue
                ...
                team = str(ev.get('team') or '').strip().lower()
                estilos = []
                if equipo_filtro and equipo_filtro!= "Ninguno" and team and team == equipo_filtro.strip().lower():
                    estilos.append("text-decoration:underline; text-decoration-thickness:2px")
                if ganador and team and team == ganador:
...
def formatear_partido(row, equipo_filtro=None, cuota_tipo=None, goles_txt=""):
    ...
    return cuotas_html + partido_html + goles_html
...
for liga in liga_sel:
    for temp in temp_sel:
        todos_eventos += cargar_eventos(liga, temp)
...
if modo_vista == "Jornadas":
    ...
    for m in todos_eventos:
        ...
        for ev in m['events']:
            ...
            if ev.get('team'): player_teams[ev['player']][ev['team']] += 1
            else: player_teams[ev['player']][ht] += 0.5; player_teams[ev['player']][at] += 0.5
...
##################################################################################################################################################################################################################################
    ...
    # --- PARCHE: evita crash del DOM ---
    MAX_FILAS = 150
    if len(df_final) > MAX_FILAS:
        st.warning(f"Mostrando {MAX_FILAS} de {len(df_final)} partidos")
        df_final = df_final.head(MAX_FILAS)
    ...
    if tipo_clasif == "Tabla":
        ...
        if equipos_grafica:
            ...
            df_html = None
########### AJUSTAR tamaño letra CSS general de la tabla ########################
css = """
<style>
table {
...
if df_html is not None:
    ...
    for i, line in enumerate(lines):
        ...
        html_table_final = '\n'.join(lines)
    st.components.v1.html(css + html_table_final, height=900, scrolling=True)
