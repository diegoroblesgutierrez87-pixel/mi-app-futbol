   ...
   ############################
    if st.button("1ª España 26/27 - BAJAR Y SUBIR A GITHUB", use_container_width=True, key="btn_1esp_2627_github_FINAL_V4"):
        ...
        for i, fx in enumerate(fixtures):
            ...
            if k in existentes and fid in set_fids: prog.progress((i+1)/len(fixtures), text=f"Ya completo {home}-{away}"); continue
    ...
    if st.button("2ª España 26/27 - BAJAR Y SUBIR A GITHUB", use_container_width=True, key="btn_2esp_2627_github_FINAL_V4"):
        ...
        if nuevos:
            ...
            if goles: pd.DataFrame(goles).to_csv(FILE_GOLES, mode='a', header=not FILE_GOLES.exists() or FILE_GOLES.stat().st_size==0, index=False)
            st.success(f"✅ 2ª: {len(nuevos)}"); push_csv_a_github(FILE_CUR, "partidos_2627_actual.csv"); push_csv_a_github(FILE_GOLES, "goles_2627_actual.csv")
        st.cache_data.clear()
