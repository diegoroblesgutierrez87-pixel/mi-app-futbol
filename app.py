...
with col_b:
    ...
    if trigger_2627 or st.button("🔄 Actualizar 26/27", use_container_width=True, key="btn_2627_final_unico"):
        ...
        st.session_state.ultima_descarga = "2627"
        total=len(MAPA_2627); step=0
###############################################################
        for nom, lid in MAPA_2627.items():
            if should_stop or st.session_state.get('pausa_descarga'): break
            step+=1; prog.progress(step/total, text=f"{nom} {TEMPORADA} | Req:{req[0]} | Q:{_quedan} | Completos:{len(existentes_completos)}")
            log_terminal(f"[{step}/{total}] {nom} {TEMPORADA} START req:{req[0]}")
            try:
                time.sleep(0.4)
                ...
                if r.status_code!=200: log_terminal(f" ERR {nom} {r.status_code}"); continue
                fixtures=r.json().get("response", [])
            except Exception as e: log_terminal(f" EXC {nom} {e}"); continue

            if not fixtures: log_terminal(f" 0 fixtures {nom}"); continue

            for fx in fixtures:
                if st.session_state.get('pausa_descarga'): break
                ...
                row={"Date":date_str,"League":nom,"Season":f"{TEMPORADA}/{TEMPORADA+1}","HomeTeam":home,"AwayTeam":away,"FTHG":ft_h,"FTAG":ft_a,"HTHG":ht_h,"HTAG":ht_a,"FTR":ftr,
                ...
                try:
                    time.sleep(0.4); rs=_req.get("https://v3.football.api-sports.io/fixtures/statistics", headers={"x-apisports-key": API_KEY}, params={"fixture": fx["fixture"]["id"]}, timeout=20); req[0]+=1
                    if rs.status_code==200 and len(rs.json().get("response",[]))==2:
                        for j, td in enumerate(rs.json()["response"]):
                            ...
                            if j==0: row["HS"]=sd.get("Total Shots",0) or 0; row["HST"]=sd.get("Shots on Goal",0) or 0; row["HF"]=sd.get("Fouls",0) or 0; row["HC"]=sd.get("Corner Kicks",0) or 0; row["HY"]=sd.get("Yellow Cards",0) or 0; row["HR"]=sd.get("Red Cards",0) or 0; row["HomePasses"]=sd.get("Total passes",0) or sd.get("Passes accurate",0) or 0; row["HomePos"]=str(sd.get("Ball Possession","")).replace("%","") or 0; row["HomeSaves"]=sd.get("Goalkeeper Saves",0) or 0
                            else: row["AS"]=sd.get("Total Shots",0) or 0; row["AST"]=sd.get("Shots on Goal",0) or 0; row["AF"]=sd.get("Fouls",0) or 0; row["AC"]=sd.get("Corner Kicks",0) or 0; row["AY"]=sd.get("Yellow Cards",0) or 0; row["AR"]=sd.get("Red Cards",0) or 0; row["AwayPasses"]=sd.get("Total passes",0) or sd.get("Passes accurate",0) or 0; row["AwayPos"]=str(sd.get("Ball Possession","")).replace("%","") or 0; row["AwaySaves"]=sd.get("Goalkeeper Saves",0) or 0
                except: pass
                ...
                try:
                    time.sleep(0.4); rp=_req.get("https://v3.football.api-sports.io/fixtures/players", headers={"x-apisports-key": API_KEY}, params={"fixture": fx["fixture"]["id"]}, timeout=20); req[0]+=1
                    if rp.status_code==200:
                        for team_data in rp.json().get("response",[]):
                            for pl in team_data.get("players",[]):
                                p=pl.get("player",{}); s=pl.get("statistics",[{}])[0]
                                nuevos_jug.append({"Date":date_str,"League":nom,"HomeTeam":home,"AwayTeam":away,"jugador":p.get("name"),"equipo":team_data["team"]["name"].upper(),"minutos":s.get("games",{}).get("minutes") or 0,"partidos":1,"fixture_id": fx["fixture"]["id"]})
                except: pass
    ...
    with col_c:
        ...
        if trigger_2226 or st.button("⬇ BAJAR LIGAS ESPECIFICAS", use_container_width=True, key="btn_especificas_final_unico"):
            ...
            st.session_state.pausa_descarga = False
            st.session_state.ultima_descarga = "2226"

            def _check_quota():
                try:
                    rr = _req.get("https://v3.football.api-sports.io/status", headers={"x-apisports-key": API_KEY}, timeout=15)
                    if rr.status_code==200:
                        j=rr.json(); gast=j.get('response',{}).get('requests',{}).get('current',0); lim=j.get('response',{}).get('requests',{}).get('limit_day',7500); return int(lim-gast), gast
                    h=rr.headers.get('x-ratelimit-requests-remaining')
                    if h: return int(str(h).strip()), 0
                except Exception as e:
                    log_terminal(f"QUOTA ERROR {e}")
                    return 7500, 0
                return 7500, 0

            _quedan, _gast = _check_quota()
            log_terminal(f"QUOTA {_quedan} restantes gastadas {_gast}")
            st.info(f"Quota: {_quedan} restantes / Gastadas: {_gast}")

            if _quedan<50:
                st.error(f"⛔ Solo {_quedan}/7500 - resetea 02:00 Madrid")
                st.stop()

            MAPA_TU_LIGA = {"Bundesliga":78,"2. Bundesliga":79,"Bundesliga Femenina":82,"Saudi Professional League":307,"Saudi First Division League":308,"Bundesliga Austria":218,"2. Liga Austria":219,"Premier League Bahrein":400,"Jupiler Pro League":144,"Challenger Pro League":145,"Chinese Super League":169,"China League One":170,"Cyprus League":318,"K League 1":292,"K League 2":293,"J1 League":98,"J2 League":99,"Superliga Dinamarca":119,"UAE League":301,"Premiership Escocia":179,"LaLiga EA Sports":140,"LaLiga Hypermotion":141,"Primera Federacion G1":435,"Primera Federacion G2":436,"Liga F":148,"Ligue 1":61,"Ligue 2":62,"Super League Grecia":197,"Super League 2 Grecia":196,"Premier League":39,"Singapore Premier League":366,"WSL":44,"WSL 2":45,"Serie A Italia":135,"Serie B Italia":136,"Liga Portugal":94,"Eredivisie":88,"Eliteserien":103,"Liga Portugal 2":95,"Süper Lig":203,"1. Lig":204,"V.League 1":340,"NB I":271,"Super League Malasia":278,"Liga 1 Indonesia":274,"Liga 2 Indonesia":276,"Thai League 1":290,"Thai League 2":291,"Nike Liga":332,"Botola Pro":200,"Ligat ha'Al":383}
            ...
            if not df_base.empty:
                try:
                    d=df_base.copy(); d["Date"]=pd.to_datetime(d["Date"], dayfirst=True, errors='coerce').dt.strftime("%d/%m/%Y"); d["HomeTeam"]=d["HomeTeam"].astype(str).str.upper().str.strip(); d["AwayTeam"]=d["AwayTeam"].astype(str).str.upper().str.strip(); d["League"]=d.get("League", pd.Series([""]*len(d))).astype(str); d["Season"]=d.get("Season", pd.Series([""]*len(d))).astype(str); existentes.update(zip(d["Date"], d["HomeTeam"], d["AwayTeam"], d["League"], d["Season"]))
                    log_terminal(f"EXISTENTES CARGADOS {len(existentes)}")
                except Exception as e:
                    log_terminal(f"ERROR EXISTENTES {e}")

            PROG_FILE = "progreso_2226.json"
            CACHE_FILE = "cache_fixtures_2226.json"
            progreso = {}
            cache_fixtures = {}
            if os.path.exists(CACHE_FILE):
                try: cache_fixtures = json.loads(open(CACHE_FILE, encoding="utf-8").read())
                except: cache_fixtures = {}
            if os.path.exists(PROG_FILE):
                try: progreso = json.loads(open(PROG_FILE, encoding="utf-8").read())
                except: progreso = {}
            lista_ligas_orden = list(MAPA_TU_LIGA.keys())
            ...
            for nom, lid in MAPA_TU_LIGA.items():
                ...
                for y in TEMPORADAS:
                    ...
                    if progreso:
                        try:
                            prog_nom = progreso.get("nom"); prog_y = progreso.get("y")
                            if prog_nom in lista_ligas_orden:
                                idx_actual = lista_ligas_orden.index(nom)*len(TEMPORADAS) + TEMPORADAS.index(y)
                                idx_prog = lista_ligas_orden.index(prog_nom)*len(TEMPORADAS) + TEMPORADAS.index(prog_y)
                                if idx_actual < idx_prog:
                                    step+=1; prog.progress(step/total, text=f"Saltando {nom} {y} ya hecho..."); continue
                        except: pass
                    step+=1; prog.progress(step/total, text=f"{nom} {y} {req[0]}/7500 Q:{_quedan}")
                    cache_key = f"{lid}_{y}"
                    fixtures = None
                    # Si temporada terminada y ya la tenemos en cache, no gastamos
                    if y <= 2024 and cache_key in cache_fixtures:
                        fixtures = cache_fixtures[cache_key]
                        log_terminal(f"CACHE {nom} {y} -> {len(fixtures)} partidos (0 req)")
                    else:
                        log_terminal(f"{nom} {y} START req:{req[0]}")
                        try:
                            ...
                            if r.status_code!=200:
                                log_terminal(f"FIXTURES {nom} {y} status {r.status_code} {r.text[:100]}"); continue
                            fixtures=r.json().get("response", [])
                            log_terminal(f"FIXTURES {nom} {y} -> {len(fixtures)} partidos")
                            if fixtures:
                                cache_fixtures[cache_key]=fixtures
                                try: open(CACHE_FILE,"w",encoding="utf-8").write(json.dumps(cache_fixtures))
                                except: pass
                        except Exception as e:
                            log_terminal(f"FIXTURES EXC {nom} {y} {e}"); continue
                    ...
                    for fx in fixtures:
                        if st.session_state.get('pausa_descarga'):
                            ...
                            try: open(PROG_FILE,"w",encoding="utf-8").write(json.dumps({"nom":nom,"y":y,"lid":lid}))
                            except: pass
                            st.warning("⏸ Pausado - dale a Continuar"); st.stop()
                        if req[0]>=7400: should_stop=True; break
                        if fx["fixture"]["status"]["short"] not in ["FT","AET","PEN"]: continue
                        ...
                        except Exception as e: log_terminal(f"STATS EXC {e}")
                        if row["HS"]==0 and row["HST"]==0 and row["HC"]==0 and row["HF"]==0 and row["HY"]==0 and row["HR"]==0: continue
                        try:
                    ...
                    if nuevos_partidos and len(nuevos_partidos)>=10:
                        pd.DataFrame(nuevos_partidos).to_csv("ligas_2122_a_2627_SIN_DUPLICADOS.csv", mode='a', header=not os.path.exists("ligas_2122_a_2627_SIN_DUPLICADOS.csv") or os.path.getsize("ligas_2122_a_2627_SIN_DUPLICADOS.csv")==0, index=False); nuevos_partidos=[]
            if nuevos_partidos:
                pd.DataFrame(nuevos_partidos).to_csv("ligas_2122_a_2627_SIN_DUPLICADOS.csv", mode='a', header=not os.path.exists("ligas_2122_a_2627_SIN_DUPLICADOS.csv") or os.path.getsize("ligas_2122_a_2627_SIN_DUPLICADOS.csv")==0, index=False)
            try:
            ...
            if should_stop:
                ...
                st.warning("⛔ Quota casi agotada, mañana dale a Continuar y no gasta 2022-2024"); st.stop()
...
##########
 ...
 with st.expander("📥 Descargas 26/27 - FIX + AUTO GITHUB", expanded=False):
    if st.button("1ª España 26/27 - BAJAR Y SUBIR A GITHUB", use_container_width=True, key="btn_1esp_2627_github_FINAL_V4"):
        ...
        def push_csv_a_github(ruta_local, path_en_repo):
            token = str(st.secrets.get("GITHUB_TOKEN","")).strip()
            repo = str(st.secrets.get("GITHUB_REPO","")).strip()
            branch = str(st.secrets.get("GITHUB_BRANCH","main")).strip()
            if not token or not repo:
                st.warning("Falta GITHUB_TOKEN/GITHUB_REPO en Secrets")
                return False
            try:
                content = pathlib.Path(ruta_local).read_bytes()
                b64 = base64.b64encode(content).decode()
                url = f"https://api.github.com/repos/{repo}/contents/{path_en_repo}"
                headers = {"Authorization": f"Bearer {token}", "Accept":"application/vnd.github+json"}
                r_get = _req.get(url, headers=headers, params={"ref":branch}, timeout=15)
                sha = r_get.json().get("sha") if r_get.status_code==200 else None
                payload = {"message": f"auto update {path_en_repo} {pd.Timestamp.now().strftime('%d/%m/%Y %H:%M')}", "content": b64, "branch": branch}
                if sha: payload["sha"]=sha
                r_put = _req.put(url, headers=headers, json=payload, timeout=20)
                if r_put.status_code in [200,201]:
                    st.success(f"✅ GitHub actualizado: {path_en_repo}")
                    return True
                else:
                    st.error(f"GitHub error {r_put.status_code}: {r_put.text[:600]}")
                    return False
            except Exception as e:
        ...
        if FILE_CUR.exists():
            try:
                d = pd.read_csv(FILE_CUR, usecols=lambda c: c in ['Date','HomeTeam','AwayTeam','HS','fixture_id'], on_bad_lines='skip')
                if 'fixture_id' in d.columns: set_fids.update(d['fixture_id'].dropna().astype(int).astype(str).tolist())
                for _, r in d.iterrows():
                    try:
                        k = (pd.to_datetime(r['Date'], dayfirst=True).strftime("%d/%m/%Y"), normaliza(r['HomeTeam']), normaliza(r['AwayTeam']))
        ...
        nuevos=[]; nuevos_goles_total=[]
        for i, fx in enumerate(fixtures_ft):
            fid=str(fx["fixture"]["id"]); date_str=pd.to_datetime(fx["fixture"]["date"][:10]).strftime("%d/%m/%Y")
            home=normaliza(fx["teams"]["home"]["name"]); away=normaliza(fx["teams"]["away"]["name"]); k=(date_str,home,away)
            if k in existentes and existentes[k] and fid in set_fids:
                prog.progress((i+1)/len(fixtures_ft), text=f"Ya completo {home}-{away}"); continue
            prog.progress((i+1)/len(fixtures_ft), text=f"Bajando {home} vs {away}")
            ft_h=fx["goals"]["home"] or 0; ft_a=fx["goals"]["away"] or 0; ht_h=fx["score"]["halftime"]["home"] or 0; ht_a=fx["score"]["halftime"]["away"] or 0
            ...
            try:
                ...
                if rs.status_code==200 and len(rs.json().get("response",[]))==2:
                    for j, td in enumerate(rs.json()["response"]):
                        sd={s["type"]:s["value"] for s in td["statistics"] if s["value"] is not None}
                        if j==0:
                            row["HS"]=sd.get("Total Shots",0) or 0; row["HST"]=sd.get("Shots on Goal",0) or 0; row["HC"]=sd.get("Corner Kicks",0) or 0; row["HY"]=sd.get("Yellow Cards",0) or 0; row["HF"]=sd.get("Fouls",0) or 0; row["HR"]=sd.get("Red Cards",0) or 0
                            row["HomePasses"]=sd.get("Total passes",0) or sd.get("Passes accurate",0) or 0; row["HomePos"]=str(sd.get("Ball Possession","")).replace("%","") or 0; row["HomeSaves"]=sd.get("Goalkeeper Saves",0) or 0
                        else:
                            row["AS"]=sd.get("Total Shots",0) or 0; row["AST"]=sd.get("Shots on Goal",0) or 0; row["AC"]=sd.get("Corner Kicks",0) or 0; row["AY"]=sd.get("Yellow Cards",0) or 0; row["AF"]=sd.get("Fouls",0) or 0; row["AR"]=sd.get("Red Cards",0) or 0
                            row["AwayPasses"]=sd.get("Total passes",0) or sd.get("Passes accurate",0) or 0; row["AwayPos"]=str(sd.get("Ball Possession","")).replace("%","") or 0; row["AwaySaves"]=sd.get("Goalkeeper Saves",0) or 0
            except: pass
            try:
                ...
                if rs_h.status_code==200:
                    for td in rs_h.json().get("response",[]):
                        ...
                        sd={s["type"]: s["value"] for s in td["statistics"] if s["value"] is not None}
                        if is_home:
                            row[f"HS{suf}"]=sd.get("Total Shots",0) or 0; row[f"HST{suf}"]=sd.get("Shots on Goal",0) or 0; row[f"HomePasses{suf}"]=sd.get("Total passes",0) or 0; row[f"HomePos{suf}"]=str(sd.get("Ball Possession","")).replace("%","") or 0
                        else:
                            row[f"AS{suf}"]=sd.get("Total Shots",0) or 0; row[f"AST{suf}"]=sd.get("Shots on Goal",0) or 0; row[f"AwayPasses{suf}"]=sd.get("Total passes",0) or 0; row[f"AwayPos{suf}"]=str(sd.get("Ball Possession","")).replace("%","") or 0
            except: pass
            try:
            ...
            try:
                ...
                if re_.status_code==200:
                    for ev in re_.json().get("response", []):
                        if ev["type"]=="Goal":
                            nuevos_goles_total.append({"Date":date_str,"League":LIGA_NOM,"Season":f"{Y}/{Y+1}","HomeTeam":home,"AwayTeam":away,"minuto":ev["time"]["elapsed"],"parte":"1P" if ev["time"]["elapsed"]<=45 else "2P","goleador":ev["player"]["name"],"asistente":ev["assist"]["name"] or "","equipo":ev["team"]["name"].upper(),"tipo":ev["detail"],"fixture_id":fx["fixture"]["id"]})
            except: pass
            nuevos.append(row)
        if nuevos:
            df_new=pd.DataFrame(nuevos); df_new.to_csv(FILE_CUR, mode='a', header=not FILE_CUR.exists() or FILE_CUR.stat().st_size==0, index=False)
            df_all=pd.read_csv(FILE_CUR, on_bad_lines='skip'); df_all=df_all.sort_values('HS').drop_duplicates(subset=['Date','HomeTeam','AwayTeam'], keep='last'); df_all.to_csv(FILE_CUR, index=False)
            if nuevos_goles_total: pd.DataFrame(nuevos_goles_total).to_csv(FILE_GOLES, mode='a', header=not FILE_GOLES.exists() or FILE_GOLES.stat().st_size==0, index=False)
            st.success(f"✅ 1ª: {len(nuevos)}"); push_csv_a_github(FILE_CUR, "partidos_2627_actual.csv"); push_csv_a_github(FILE_GOLES, "goles_2627_actual.csv")
        st.cache_data.clear()

    if st.button("2ª España 26/27 - BAJAR Y SUBIR A GITHUB", use_container_width=True, key="btn_2esp_2627_github_FINAL_V4"):
        import requests as _req, time, pathlib, pandas as pd, base64
        ...
        BASE = pathlib.Path(__file__).parent; FILE_CUR = BASE / "partidos_2627_actual.csv"; FILE_GOLES = BASE / "goles_2627_actual.csv"
        def push_csv_a_github(ruta_local, path_en_repo):
            token = str(st.secrets.get("GITHUB_TOKEN","")).strip(); repo = str(st.secrets.get("GITHUB_REPO","")).strip(); branch = str(st.secrets.get("GITHUB_BRANCH","main")).strip()
            if not token or not repo: return False
            try:
                b64 = base64.b64encode(pathlib.Path(ruta_local).read_bytes()).decode()
                url = f"https://api.github.com/repos/{repo}/contents/{path_en_repo}"; h={"Authorization": f"Bearer {token}", "Accept":"application/vnd.github+json"}
                r_get = _req.get(url, headers=h, params={"ref":branch}, timeout=15); sha = r_get.json().get("sha") if r_get.status_code==200 else None
                pay={"message": f"update {path_en_repo}", "content": b64, "branch": branch}
                if sha: pay["sha"]=sha
        ...
        if FILE_CUR.exists():
            try:
                ...
                if 'fixture_id' in d.columns: set_fids.update(d['fixture_id'].dropna().astype(int).astype(str).tolist())
                for _, r in d.iterrows():
                    try: k=(pd.to_datetime(r['Date'], dayfirst=True).strftime("%d/%m/%Y"), normaliza(r['HomeTeam']), normaliza(r['AwayTeam'])); existentes[k]=float(r.get('HS',0))>0
                    except: pass
            except: pass
        prog = st.progress(0); r=_req.get("https://v3.football.api-sports.io/fixtures", headers={"x-apisports-key": API_KEY}, params={"league": LIGA_ID, "season": Y}, timeout=30)
        fixtures=[f for f in r.json().get("response",[]) if f["fixture"]["status"]["short"] in ["FT","AET","PEN"]]
        nuevos=[]; goles=[]
        for i, fx in enumerate(fixtures):
            fid=str(fx["fixture"]["id"]); ds=pd.to_datetime(fx["fixture"]["date"][:10]).strftime("%d/%m/%Y"); h=normaliza(fx["teams"]["home"]["name"]); a=normaliza(fx["teams"]["away"]["name"]); k=(ds,h,a)
            if k in existentes and fid in set_fids: continue
            row={"Date":ds,"League":LIGA_NOM,"Season":f"{Y}/{Y+1}","HomeTeam":h,"AwayTeam":a,"FTHG":fx["goals"]["home"] or 0,"FTAG":fx["goals"]["away"] or 0,"HTHG":fx["score"]["halftime"]["home"] or 0,"HTAG":fx["score"]["halftime"]["away"] or 0,"FTR":"H","B365H":0,"B365D":0,"B365A":0,"HS":0,"AS":0,"HST":0,"AST":0,"HF":0,"AF":0,"HC":0,"AC":0,"HY":0,"AY":0,"HR":0,"AR":0,"HomePasses":0,"AwayPasses":0,"HomeSaves":0,"AwaySaves":0,"HomePos":0,"AwayPos":0,"HS_1P":0,"AS_1P":0,"HST_1P":0,"AST_1P":0,"HF_1P":0,"AF_1P":0,"HC_1P":0,"AC_1P":0,"HY_1P":0,"AY_1P":0,"HR_1P":0,"AR_1P":0,"HomePasses_1P":0,"AwayPasses_1P":0,"HomePos_1P":0,"AwayPos_1P":0,"HS_2P":0,"AS_2P":0,"HST_2P":0,"AST_2P":0,"HF_2P":0,"AF_2P":0,"HC_2P":0,"AC_2P":0,"HY_2P":0,"AY_2P":0,"HR_2P":0,"AR_2P":0,"HomePasses_2P":0,"AwayPasses_2P":0,"HomePos_2P":0,"AwayPos_2P":0,"fixture_id":fx["fixture"]["id"]}
            row["FTR"]="H" if row["FTHG"]>row["FTAG"] else "A" if row["FTAG"]>row["FTHG"] else "D"
            try:
                time.sleep(0.35); rs=_req.get("https://v3.football.api-sports.io/fixtures/statistics", headers={"x-apisports-key": API_KEY}, params={"fixture": fx["fixture"]["id"]}, timeout=20)
                if rs.status_code==200 and len(rs.json().get("response",[]))==2:
                    for j, td in enumerate(rs.json()["response"]):
                        sd={s["type"]:s["value"] for s in td["statistics"] if s["value"] is not None}
                        if j==0: row["HS"]=sd.get("Total Shots",0) or 0; row["HST"]=sd.get("Shots on Goal",0) or 0; row["HC"]=sd.get("Corner Kicks",0) or 0; row["HomePasses"]=sd.get("Total passes",0) or 0; row["HomePos"]=str(sd.get("Ball Possession","")).replace("%","") or 0
                        else: row["AS"]=sd.get("Total Shots",0) or 0; row["AST"]=sd.get("Shots on Goal",0) or 0; row["AC"]=sd.get("Corner Kicks",0) or 0; row["AwayPasses"]=sd.get("Total passes",0) or 0; row["AwayPos"]=str(sd.get("Ball Possession","")).replace("%","") or 0
            except: pass
            try:
                time.sleep(0.35); rs_h=_req.get("https://v3.football.api-sports.io/fixtures/statistics", headers={"x-apisports-key": API_KEY}, params={"fixture": fx["fixture"]["id"], "half": "true"}, timeout=20)
                if rs_h.status_code==200:
                    for td in rs_h.json().get("response",[]):
                        suf="_1P" if str(td.get("half")).lower() in ["1","first"] else "_2P"; sd={s["type"]: s["value"] for s in td["statistics"] if s["value"] is not None}
                        is_home=td["team"]["id"]==fx["teams"]["home"]["id"]
                        if is_home: row[f"HS{suf}"]=sd.get("Total Shots",0) or 0; row[f"HomePasses{suf}"]=sd.get("Total passes",0) or 0
                        else: row[f"AS{suf}"]=sd.get("Total Shots",0) or 0; row[f"AwayPasses{suf}"]=sd.get("Total passes",0) or 0
            except: pass
            try:
                time.sleep(0.35); ro=_req.get("https://v3.football.api-sports.io/odds", headers={"x-apisports-key": API_KEY}, params={"fixture": fx["fixture"]["id"],"bookmaker":8}, timeout=20)
                if ro.status_code==200 and ro.json().get("response",[]) and ro.json()["response"][0].get("bookmakers"):
            ...
            try:
                ...
                if re_.status_code==200:
                    for ev in re_.json().get("response", []):
                        if ev["type"]=="Goal": goles.append({"Date":ds,"League":LIGA_NOM,"HomeTeam":h,"AwayTeam":a,"minuto":ev["time"]["elapsed"],"goleador":ev["player"]["name"],"asistente":ev["assist"]["name"] or "","equipo":ev["team"]["name"].upper(),"fixture_id":fx["fixture"]["id"]})
            except: pass
            nuevos.append(row); prog.progress((i+1)/len(fixtures))
        if nuevos:
            ...
            st.success(f"✅ 2ª: {len(nuevos)} partidos"); push_csv_a_github(FILE_CUR, "partidos_2627_actual.csv"); push_csv_a_github(FILE_GOLES, "goles_2627_actual.csv")
        st.cache_data.clear()
