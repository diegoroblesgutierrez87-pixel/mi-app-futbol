...
######################################## FIN CHECKLIST KOREA
if st.button("🇰🇷 KOREA K1+K2 2026 - GUARDA AUTOMATICO", use_container_width=True):
    import requests as _req, time, pathlib, pandas as pd, base64, os
    ...
    if FILE_CUR.exists():
        ...
        existentes=set(df_ex["fixture_id"].astype(str))
        st.info(f"Ya tienes {len(df_ex)}")

    def push_a_github():
        if not GITHUB_TOKEN or not GITHUB_REPO:
            st.warning("Falta GITHUB_TOKEN en secrets, no puedo subir auto. Descarga a mano.")
            return
        try:
            for fp in [FILE_CUR, FILE_GOLES]:
            ...
            st.success("SUBIDO A GITHUB AUTOMATICO - Ya no se borra nunca")
        except Exception as e:
            st.error(f"Error subiendo a Github: {e}")

    LIGAS=[(292,"K League 1",2026),(293,"K League 2",2026)]
    req=[0]; total_nuevos=0
    for LIGA_ID,NOMBRE,_ in LIGAS:
        data=_req.get("https://v3.football.api-sports.io/fixtures", headers={"x-apisports-key": API_KEY}, params={"league":LIGA_ID,"season":2026}, timeout=30).json().get("response",[])
        ...
        for fx in fixtures:
            if req[0]>=950: push_a_github(); st.warning("Limite 950, dale otra vez"); st.stop()
            fid=str(fx["fixture"]["id"])
                ...
                if re_.status_code==200:
                    ...
                    for ev in re_.json().get("response",[]):
                        if ev["type"] in ["Goal","Card"]:
                            todos_goles.append({"Date":date_str,"League":NOMBRE,"Season":SEASON_LABEL,"HomeTeam":home,"AwayTeam":away,"minuto":ev["time"]["elapsed"],"parte":"1P" if (ev["time"]["elapsed"] or 0)<=45 else "2P","goleador":ev["player"]["name"] if ev["type"]=="Goal" else "","asistente":ev["assist"]["name"] if ev["type"]=="Goal" else "","jugador_tarjeta":ev["player"]["name"] if ev["type"]=="Card" else "","equipo":normaliza(ev["team"]["name"]),"tipo":ev["detail"],"fixture_id": fid})
                    if todos_goles:
