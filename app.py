    ...
    if st.button("Ligas 26/27 - FIX TOTAL + GOLES + MINUTOS", use_container_width=True, key="btn_1esp_2627_FIX_TOTAL_V6_COMPAT"):
        ...
        for idx_liga in range(liga_start_idx, len(lista_ligas)):
            ...
            if st.session_state.pausa_2627:
                ...
                st.warning(f"⏸ Pausado en {nom}. Dale a ▶ Continuar 26/27 para seguir."); st.stop()
            try:
                time.sleep(0.35)
            ...
            for fx in fixtures:
                if st.session_state.pausa_2627:
                    ...
                    st.warning(f"⏸ Pausado en {nom} | {fx['teams']['home']['name']} vs {fx['teams']['away']['name']}"); st.stop()
                if fx["fixture"]["status"]["short"] not in ["FT","AET","PEN"]: continue
                        ...
                        try:
                            ...
                            if re_.status_code==200:
                                ...
                                for ev in re_.json().get("response", []):
                                    if ev["type"]=="Goal":
                                        tmp_g.append({"Date":date_str,"League":nom,"Season":f"{TEMPORADA}/{TEMPORADA+1}","HomeTeam":home,"AwayTeam":away,"minuto":ev["time"]["elapsed"],"parte":"1P" if (ev["time"]["elapsed"] or 0)<=45 else "2P","goleador":ev["player"]["name"],"asistente":ev["assist"]["name"] or "","jugador_tarjeta":"","equipo":normaliza(ev["team"]["name"]),"tipo":ev["detail"],"fixture_id": fx["fixture"]["id"]})
                                    elif ev["type"]=="Card":
                                        tmp_g.append({"Date":date_str,"League":nom,"Season":f"{TEMPORADA}/{TEMPORADA+1}","HomeTeam":home,"AwayTeam":away,"minuto":ev["time"]["elapsed"],"parte":"1P" if (ev["time"]["elapsed"] or 0)<=45 else "2P","goleador":"","asistente":"","jugador_tarjeta":ev["player"]["name"],"equipo":normaliza(ev["team"]["name"]),"tipo":ev["detail"],"fixture_id": fx["fixture"]["id"]})
                                if tmp_g:
                                    pd.DataFrame(tmp_g).to_csv(FILE_GOLES, mode='a', header=not FILE_GOLES.exists() or FILE_GOLES.stat().st_size==0, index=False)
                                    goles_fids.add(fid)
                        except: pass
                        continue
...
########################################boton J1 2026/27 ESTE AÑO - SOLO JAPON NUEVA TEMPORADA
    if st.button("J1 2026/27 ESTE AÑO - SOLO JAPON", use_container_width=True, key="btn_j1_2027_solo_este_anio_final"):
        ...
        LIGA_ID = 98
        NOMBRE_LIGA = "J1 League"
        SEASON = 2027 # J1 nueva que empezó 07/08/2026
        BASE = pathlib.Path(__file__).parent
        FILE_CUR = BASE / "partidos_2627_actual.csv"
        FILE_GOLES = BASE / "goles_2627_actual.csv"
        ...
        try:
            time.sleep(0.4); r=_req.get("https://v3.football.api-sports.io/fixtures", headers={"x-apisports-key": API_KEY}, params={"league":LIGA_ID,"season":SEASON}, timeout=30); req[0]+=1
            fixtures=[f for f in r.json().get("response",[]) if str(f["fixture"]["id"]) not in existentes and f["fixture"]["status"]["short"] in ["FT","AET","PEN"]]
        except Exception as e:
            st.error(f"Error API fixtures: {e}"); fixtures=[]
        ...
        for fx in fixtures:
                ...
                if rs.status_code==200:
                    ...
                    if len(resp)==2:
                        for j,td in enumerate(resp):
                            ...
                            if j==0:
                                row["HS"]=get_int("Total Shots"); row["HST"]=get_int("Shots on Goal"); row["HF"]=get_int("Fouls"); row["HC"]=get_int("Corner Kicks"); row["HY"]=get_int("Yellow Cards"); row["HR"]=get_int("Red Cards"); row["HomePasses"]=get_int("Total passes"); row["HomeSaves"]=get_int("Goalkeeper Saves"); row["HomePos"]=get_int("Ball Possession")
                            else:
                        ...
                        if row["HS"]!=0 or row["HC"]!=0 or row["HomePasses"]!=0:
                            tiene_stats=True
            except: pass
            if not tiene_stats:
                prog.progress(min(req[0]/490,0.99), text=f"J1 SIN STATS {home} vs {away} {date_str} -> SKIP {req[0]}/490")
                continue
            ...
            try:
                ...
                if re_.status_code==200:
                    for ev in re_.json().get("response",[]):
                        if ev["type"]=="Goal":
                            todos_goles.append({"Date":date_str,"League":NOMBRE_LIGA,"Season":"2026/27","HomeTeam":home,"AwayTeam":away,"minuto":ev["time"]["elapsed"],"parte":"1P" if (ev["time"]["elapsed"] or 0)<=45 else "2P","goleador":ev["player"]["name"],"asistente":ev["assist"]["name"] or "","jugador_tarjeta":"","equipo":normaliza(ev["team"]["name"]),"tipo":ev["detail"],"fixture_id": fid})
                        elif ev["type"]=="Card":
                            todos_goles.append({"Date":date_str,"League":NOMBRE_LIGA,"Season":"2026/27","HomeTeam":home,"AwayTeam":away,"minuto":ev["time"]["elapsed"],"parte":"1P" if (ev["time"]["elapsed"] or 0)<=45 else "2P","goleador":"","asistente":"","jugador_tarjeta":ev["player"]["name"],"equipo":normaliza(ev["team"]["name"]),"tipo":ev["detail"],"fixture_id": fid})
            except: pass
            todos_partidos.append(row); existentes.add(str(fid))
            prog.progress(min(req[0]/490,0.99), text=f"J1 2026/27 {len(todos_partidos)} OK {home} vs {away} {date_str} {req[0]}/490 req")
        ...
        st.success(f"J1 2026/27 {len(todos_partidos)} partidos REALES J1-J4 {len(todos_goles)} eventos {req[0]} req"); st.cache_data.clear(); st.rerun()
################################################
