...
with st.expander("📥 Descargas 26/27 - FIX + AUTO GITHUB", expanded=False):
    ...
    if st.button("1ª España 26/27 - FIX TOTAL 1P/2P + GOLES + MINUTOS", use_container_width=True, key="btn_1esp_2627_FIX_TOTAL_V5"):
        ...
        for i, fx in enumerate(fixtures):
            ...
            prog.progress((i+1)/len(fixtures), text=f"Bajando {home} vs {away} - {i+1}/{len(fixtures)}")
            ft_h=fx["goals"]["home"] or 0; ft_a=fx["goals"]["away"] or 0; ht_h=fx["score"]["halftime"]["home"] or 0; ht_a=fx["score"]["halftime"]["away"] or 0
                ...
                if rp.status_code==200:
                    for team_data in rp.json().get("response",[]):
                        ...
                        for pl in team_data.get("players",[]):
                            p=pl.get("player",{}); s=pl.get("statistics",[{}])[0]
                            jugadores.append({
                                "Date":date_str,"League":LIGA_NOM,"Season":f"{Y}/{Y+1}",
                                "HomeTeam":home,"AwayTeam":away,
                                "jugador":p.get("name"),"equipo":eq_name,
                                "minutos":s.get("games",{}).get("minutes") or 0,
                                "rating":s.get("games",{}).get("rating") or 0,
                                "titular": s.get("games",{}).get("position")!= "G" or True,
                                "fixture_id": fx["fixture"]["id"]
                            })
            except: pass
        ...
        if nuevos:
            ...
            if jugadores:
                pd.DataFrame(jugadores).to_csv(FILE_JUG, mode='a', header=not FILE_JUG.exists() or FILE_JUG.stat().st_size==0, index=False)
                df_j=pd.read_csv(FILE_JUG, on_bad_lines='skip'); df_j=df_j.drop_duplicates(subset=['fixture_id','jugador'], keep='last'); df_j.to_csv(FILE_JUG, index=False)
            st.success(f"✅ 1ª FIX: {len(nuevos)} partidos | {len(goles)} goles | {len(jugadores)} registros minutos")
            push_csv_a_github(FILE_CUR, "partidos_2627_actual.csv"); push_csv_a_github(FILE_GOLES, "goles_2627_actual.csv"); push_csv_a_github(FILE_JUG, "jugadores_2627_actual.csv")
        st.cache_data.clear()
...
with st.expander("📥 Descargas 26/27 - FIX + AUTO GITHUB", expanded=False):
    ...
    if st.button("1ª España 26/27 - FIX TOTAL 1P/2P + GOLES + MINUTOS", use_container_width=True, key="btn_1esp_2627_FIX_TOTAL_V5"):
        ...
        for i, fx in enumerate(fixtures):
            fid=str(fx["fixture"]["id"]); date_str=pd.to_datetime(fx["fixture"]["date"][:10]).strftime("%d/%m/%Y"); home=normaliza(fx["teams"]["home"]["name"]); away=normaliza(fx["teams"]["away"]["name"]); k=(date_str,home,away)
            # --- LOGICA NUEVA: SOLO FINALIZADO + RE-BAJAR SI INCOMPLETO ---
            if k in existentes and fid in set_fids:
                row_old = df_exist_map.get(k, {})
                completo, faltan = esta_completo_row(row_old)
                if completo:
                    prog.progress((i+1)/len(fixtures), text=f"Ya 100% completo {home}-{away}"); continue
                else:
                    log_terminal(f"RE-BAJAR 1ª {fid} {home}-{away} faltan {faltan}")
                    # lo quitamos de existentes para que se re-escriba luego
                    existentes.pop(k, None)
                    # también lo quitamos del CSV en memoria luego con drop_duplicates keep last
            prog.progress((i+1)/len(fixtures), text=f"Bajando {home} vs {away} - {i+1}/{len(fixtures)}")
            ft_h=fx["goals"]["home"] or 0; ft_a=fx["goals"]["away"] or 0; ht_h=fx["score"]["halftime"]["home"] or 0; ht_a=fx["score"]["halftime"]["away"] or 0
            row = {"Date":date_str,"League":LIGA_NOM,"Season":f"{Y}/{Y+1}","HomeTeam":home,"AwayTeam":away,"FTHG":ft_h,"FTAG":ft_a,"HTHG":ht_h,"HTAG":ht_a,"FTR":"H" if ft_h>ft_a else "A" if ft_a>ft_h else "D","B365H":0,"B365D":0,"B365A":0,"HS":0,"AS":0,"HST":0,"AST":0,"HF":0,"AF":0,"HC":0,"AC":0,"HY":0,"AY":0,"HR":0,"AR":0,"HomePasses":0,"AwayPasses":0,"HomeSaves":0,"AwaySaves":0,"HomePos":0,"AwayPos":0,"HS_1P":0,"AS_1P":0,"HST_1P":0,"AST_1P":0,"HF_1P":0,"AF_1P":0,"HC_1P":0,"AC_1P":0,"HY_1P":0,"AY_1P":0,"HR_1P":0,"AR_1P":0,"HomePasses_1P":0,"AwayPasses_1P":0,"HomePos_1P":0,"AwayPos_1P":0,"HS_2P":0,"AS_2P":0,"HST_2P":0,"AST_2P":0,"HF_2P":0,"AF_2P":0,"HC_2P":0,"AC_2P":0,"HY_2P":0,"AY_2P":0,"HR_2P":0,"AR_2P":0,"HomePasses_2P":0,"AwayPasses_2P":0,"HomePos_2P":0,"AwayPos_2P":0,"fixture_id":fx["fixture"]["id"]}
    ...
    if st.button("2ª España 26/27 - FIX TOTAL 1P/2P + GOLES + MINUTOS", use_container_width=True, key="btn_2esp_2627_FIX_TOTAL_V5"):
        ...
        for i, fx in enumerate(fixtures):
            ...
            if k in existentes and fid in set_fids:
                ...
                if completo:
                    ...
                    log_terminal(f"RE-BAJAR 2ª {fid} {home}-{away} faltan {faltan}")
                    existentes.pop(k, None)
        ...
        if nuevos:
            ...
            if goles: pd.DataFrame(goles).to_csv(FILE_GOLES, mode='a', header=not FILE_GOLES.exists() or FILE_GOLES.stat().st_size==0, index=False)
            if jugadores: pd.DataFrame(jugadores).to_csv(FILE_JUG, mode='a', header=not FILE_JUG.exists() or FILE_JUG.stat().st_size==0, index=False); df_j=pd.read_csv(FILE_JUG, on_bad_lines='skip'); df_j=df_j.drop_duplicates(subset=['fixture_id','jugador'], keep='last'); df_j.to_csv(FILE_JUG, index=False)
            st.success(f"✅ 2ª FIX: {len(nuevos)} partidos | {len(goles)} goles | {len(jugadores)} minutos")
            push_csv_a_github(FILE_CUR, "partidos_2627_actual.csv"); push_csv_a_github(FILE_GOLES, "goles_2627_actual.csv"); push_csv_a_github(FILE_JUG, "jugadores_2627_actual.csv")
        else:
            st.info("No hay partidos nuevos para 2ª o todos ya están 100% completos")
        st.cache_data.clear()
