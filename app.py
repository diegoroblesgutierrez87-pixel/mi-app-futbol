...
with st.expander("⚙ Opciones avanzadas"):
    ...
    with col_b:
        if st.button("🔄 Actualizar 26/27", type="primary", width='stretch', key="btn_2627_final_unico"):
            ...
            MAPA_TU_LIGA = {"Bundesliga":78,"2. Bundesliga":79,"Frauen-Bundesliga":86,"Saudi Professional League":307,"Primera Division":128,"Ligue 1":61,"Ligue 2":62,"Premier League":39,"Championship":40,"League One":46,"Jupiler Pro League":144,"Challenger Pro League":145,"Super League":207,"Challenge League":208,"Serie A":135,"Serie B":136,"LaLiga EA Sports":140,"LaLiga Hypermotion":141,"Primera Federacion":435,"Liga F":148,"Eredivisie":88,"Liga Portugal":94,"Liga Portugal 2":95,"Super Lig":203,"1. Lig":204,"J1 League":98,"J2 League":99,"K League 1":292,"K League 2":293,"UAE League":301,"Thai League 1":290,"Thai League 2":291,"V.League 1":340,"Eliteserien":103,"NB I":271,"Premiership":179,"Nike Liga":332,"Ligat ha'Al":383,"Botola Pro":200,"Super Liga Serbia":286,"Cyprus League":318,"Superliga Dinamarca":119,"Serie A Brasil":71,"Serie B Brasil":72}
            TEMPORADAS = [2026]
            ...
            if not df_base.empty:
                try: d=df_base.copy(); d["Date"]=pd.to_datetime(d["Date"], dayfirst=True, errors='coerce').dt.strftime("%d/%m/%Y"); d["HomeTeam"]=d["HomeTeam"].astype(str).str.upper().str.strip(); d["AwayTeam"]=d["AwayTeam"].astype(str).str.upper().str.strip(); existentes.update(zip(d["Date"], d["HomeTeam"], d["AwayTeam"], d["League"], d["Season"]))
                except: pass
            ...
            for nom, lid in MAPA_TU_LIGA.items():
                ...
                for y in TEMPORADAS:
                    if req[0]>=7400: should_stop=True; break
                    step+=1; prog.progress(step/total, text=f"{nom} {y} {req[0]}/7500"); log_terminal(f"{nom} {y} START {req[0]}")
                    try:
            ...
            st.success(f"✅ 26/27 {req[0]}/7500"); st.cache_data.clear(); st.rerun()
    ...
    if st.button("⬇ BAJAR LIGAS ESPECIFICAS", type="primary", width='stretch', key="btn_especificas_final_unico"):
        ...
        for nom, lid in MAPA_ESPECIFICAS.items():
            ...
            for y in TEMPORADAS:
                ...
                prog2.progress(0, text=f"{nom} {y} {req2[0]}/7500")
                try:
                    time.sleep(0.35); r=_req2.get("https://v3.football.api-sports.io/fixtures", headers={"x-apisports-key": API_KEY2}, params={"league": lid, "season": y}, timeout=30); req2[0]+=1
                    if int(str(r.headers.get('x-ratelimit-requests-remaining','7500')).strip())<20: should_stop=True; break
                except: continue
