...
# --- FUNCIONES FUERA - UNA SOLA VEZ, NO DENTRO DEL BOTON ---
...
#################### ligas especificas 26 27 - BLOQUE UNICO CORREGIDO
with st.expander("📥 Descargas 26/27 - FIX + AUTO GITHUB", expanded=False):
    ...
    if st.button("1ª España 26/27 - FIX TOTAL 1P/2P + GOLES + MINUTOS", use_container_width=True, key="btn_1esp_2627_FIX_TOTAL_V5"):
            ...
            try:
                ...
                if rs_h.status_code==200:
                    for td in rs_h.json().get("response",[]):
                        ...
                        if is_home: row[f"HS{suf}"]=sd.get("Total Shots",0) or 0; row[f"HST{suf}"]=sd.get("Shots on Goal",0) or 0; row[f"HC{suf}"]=sd.get("Corner Kicks",0) or 0; row[f"HY{suf}"]=sd.get("Yellow Cards",0) or 0; row[f"HF{suf}"]=sd.get("Fouls",0) or 0
                        else: row[f"AS{suf}"]=sd.get("Total Shots",0) or 0; row[f"AST{suf}"]=sd.get("Shots on Goal",0) or 0; row[f"AC{suf}"]=sd.get("Corner Kicks",0) or 0; row[f"AY{suf}"]=sd.get("Yellow Cards",0) or 0; row[f"AF{suf}"]=sd.get("Fouls",0) or 0
            except: pass
            try:
        ...
        if nuevos:
            ...
            st.success(f"✅ 1ª FIX: {len(nuevos)} partidos")
            push_csv_a_github(FILE_CUR, "partidos_2627_actual.csv")
        else:
            st.info("No hay partidos nuevos para 1ª")
        st.cache_data.clear()
    ...
    if st.button("2ª España 26/27 - FIX TOTAL 1P/2P + GOLES + MINUTOS", use_container_width=True, key="btn_2esp_2627_FIX_TOTAL_V5"):
        ...
        for i, fx in enumerate(fixtures):
            ...
            try:
                ...
                if rs.status_code==200 and len(rs.json().get("response",[]))==2:
                    for j, td in enumerate(rs.json()["response"]):
                        sd={s["type"]:s["value"] for s in td["statistics"] if s["value"] is not None}
                        if j==0: row["HS"]=sd.get("Total Shots",0) or 0; row["HST"]=sd.get("Shots on Goal",0) or 0; row["HC"]=sd.get("Corner Kicks",0) or 0
                        else: row["AS"]=sd.get("Total Shots",0) or 0; row["AST"]=sd.get("Shots on Goal",0) or 0; row["AC"]=sd.get("Corner Kicks",0) or 0
            except: pass
            try:
                time.sleep(0.35); rs_h=_req.get("https://v3.football.api-sports.io/fixtures/statistics", headers={"x-apisports-key": API_KEY}, params={"fixture": fx["fixture"]["id"], "half": "true"}, timeout=20)
                if rs_h.status_code==200:
                    for td in rs_h.json().get("response",[]):
                        is_home=td["team"]["id"]==fx["teams"]["home"]["id"]; suf="_1P" if str(td.get("half")).lower() in ["1","first","1st"] else "_2P"; sd={s["type"]: s["value"] for s in td["statistics"] if s["value"] is not None}
                        if is_home: row[f"HS{suf}"]=sd.get("Total Shots",0) or 0; row[f"HC{suf}"]=sd.get("Corner Kicks",0) or 0
                        else: row[f"AS{suf}"]=sd.get("Total Shots",0) or 0; row[f"AC{suf}"]=sd.get("Corner Kicks",0) or 0
            except: pass
            try:
                time.sleep(0.35); ro=_req.get("https://v3.football.api-sports.io/odds", headers={"x-apisports-key": API_KEY}, params={"fixture": fx["fixture"]["id"],"bookmaker":8}, timeout=20)
                if ro.status_code==200 and ro.json().get("response",[]) and ro.json()["response"][0].get("bookmakers"):
        ...
        if nuevos:
            ...
            if goles: pd.DataFrame(goles).to_csv(FILE_GOLES, mode='a', header=not FILE_GOLES.exists() or FILE_GOLES.stat().st_size==0, index=False)
            if jugadores: pd.DataFrame(jugadores).to_csv(FILE_JUG, mode='a', header=not FILE_JUG.exists() or FILE_JUG.stat().st_size==0, index=False)
            st.success(f"✅ 2ª FIX: {len(nuevos)} partidos")
            push_csv_a_github(FILE_CUR, "partidos_2627_actual.csv")
        else:
            st.info("No hay partidos nuevos para 2ª")
        st.cache_data.clear()
