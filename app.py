...
def fmt_rapido(r, eq_refs_norm, current_eq_norm, current_eq_orig):
    ...
    return f"<div style='font-family:monospace;font-size:11px;padding:8px 6px;border-bottom:2px solid #333;line-height:1.4;word-wrap:break-word;overflow-wrap:anywhere;white-space:normal;text-align:center;max-width:380px;width:100%;margin:0 auto;background:#fff'><span style='color:{col};font-weight:900'>|J{j}| {btts_txt}{loc_tag}</span>{extra}<br><span style='color:{col};font-weight:900'>{hab_u} [ {hg}-{ag} ] {aab_u}</span><br><span style='color:#555;font-weight:700'>{hab} [ {hthg}-{htag} ] {aab}</span><br><div style='color:#000;white-space:normal;word-break:break-word;line-height:1.1;display:flex;flex-direction:column;align-items:center'>{txt_mins}</div></div>"
