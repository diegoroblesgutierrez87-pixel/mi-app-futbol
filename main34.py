...
with st.expander("📋 Partidos", expanded=True):
    if equipo_filtro!= "Ninguno" and equipo2_filtro!= "Ninguno":
        ...
        with col_izq:
            ...
            st.components.v1.html(render_tabla_equipo(df1, equipo_filtro), height=700, scrolling=True)
        with col_der:
            st.caption(f"Mostrando {len(df2)} partidos de {equipo2_filtro}")
            st.components.v1.html(render_tabla_equipo(df2, equipo2_filtro), height=700, scrolling=True)
    else:
        # MODO NORMAL
        if len(df_final) > 0:
            df_final['partidos'] = df_final.apply(lambda row: formatear_partido(row, equipo_filtro, cuota_tipo, row.get('Goles','')), axis=1)
        ...
        st.components.v1.html(css + html_table, height=700, scrolling=True)



###################hasta aqui st divider######################
...
def _rachas(df_base, cond, loc):
    ...
    mapa = {
        ...
        "Gana/Empata": {'G','E'}, "Empata/Pierde": {'E','P'}, "Gana/Pierde": {'G','P'}
    }
    cs = mapa[cond]
    out = []
    for eq, g in d.groupby('Equipo'):
        ...
        # recorta si son demasiadas para no romper la tabla
        if len(jornadas_ok) > 15:
            jornadas_str = ','.join(f"J{j}" for j in jornadas_ok[:15]) + f" +{len(jornadas_ok)-15}"
        else:
            jornadas_str = ','.join(f"J{j}" for j in jornadas_ok)

        ult5 = ''.join(s[-5:])
        texto = f"{eq} | {len(s)}J | {seg} | {pct}% ({jornadas_str}) | {ult5}"
        out.append({'Equipo': texto, 'PJ': len(s), 'Seguidos': seg, '%': pct})
    return pd.DataFrame(out)

with st.expander("🔥 Filtro Rachas", expanded=False):
    ...
    if not src.empty:
        ...
        st.dataframe(
            ...
            hide_index=True,
            height=380,
            column_config={"Equipo": st.column_config.TextColumn("Equipo | PJ | Seg | % (Jornadas) | Ult5")}
        )
# ==================== FIN ====================