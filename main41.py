...
with st.expander("🎯 Creador Apuestas", expanded=False):
    ...
    df_creador_base = df[(df['League']==ca_liga) & (df['Season']==ca_temp)]
    ...
    if st.button("Generar partido", key="ca_gen", use_container_width=True) and eq1 and eq2:
        df_r = df_creador[(df_creador['Jornada']>=j1) & (df_creador['Jornada']<=j2)]
        m1 = df_r[(df_r['HomeTeam']==eq1)|(df_r['AwayTeam']==eq1)].tail(20)
        m2 = df_r[(df_r['HomeTeam']==eq2)|(df_r['AwayTeam']==eq2)].tail(20)

        if len(m1)<4 or len(m2)<4:
            st.warning("Necesitas mínimo 4 partidos")
            st.stop()

        # --- CALIBRACIÓN UNIVERSAL ---
        lg_home = df_r['FTHG'].mean(); lg_away = df_r['FTAG'].mean()
        lg_g = (lg_home + lg_away)/2
...
######fuerza
        def fuerza(eq, df_eq):
            ...
            atk = 0.75*atk + 0.25; defe = 0.75*defe + 0.25

            # --- PEDIGREE HISTÓRICO (últimas 3 temporadas) ---
            temps = sorted(df['Season'].unique())
            hist = 1.0
            if ca_temp in temps:
                idx = temps.index(ca_temp)
                prev = temps[max(0, idx-3):idx]
                pos_hist = []
                for t in prev:
                    dt = df[(df['League']==ca_liga) & (df['Season']==t)]
                    if dt.empty: continue
                    lj = dt['Jornada'].max()
                    dl = dt[dt['Jornada']==lj]
                    r = dl[(dl['HomeTeam']==eq)|(dl['AwayTeam']==eq)]
                    if not r.empty:
                        p = r['HomePos'].iloc[0] if r['HomeTeam'].iloc[0]==eq else r['AwayPos'].iloc[0]
                        pos_hist.append(p)
                if pos_hist:
       ...
       #########fuerza
        ...
        # --- PESO POR CONTEXTO DE CLASIFICACIÓN ---
        pts1 = m1['HomePtsPrev'].iloc[-1] if m1['HomeTeam'].iloc[-1]==eq1 else m1['AwayPtsPrev'].iloc[-1]
        pts2 = m2['HomePtsPrev'].iloc[-1] if m2['HomeTeam'].iloc[-1]==eq2 else m2['AwayPtsPrev'].iloc[-1]
        ...
        zona_descenso = n_eq - 2  # últimos 3
        zona_media_ini = int(n_eq*0.45)
        zona_media_fin = int(n_eq*0.70)

        if f1['pos'] <= 2 and f2['pos'] >= zona_descenso:
            peso_h *= 1.15; peso_a *= 0.88
        elif f2['pos'] <= 2 and f1['pos'] >= zona_descenso:
            peso_h *= 0.88; peso_a *= 1.15
        elif f1['pos'] <=4 and f2['pos'] <=4 and abs(pts1-pts2) <= 6:
            peso_h *= 1.10; peso_a *= 1.10
            f1['hf'] *= 1.15; f2['hf'] *= 1.15
            f1['hy'] *= 1.2; f2['hy'] *= 1.2
        elif f1['pos'] >= zona_descenso and f2['pos'] >= zona_descenso:
            peso_h *= 1.08; peso_a *= 1.08
            f1['hf'] *= 1.2; f2['hf'] *= 1.2
        elif zona_media_ini <= f1['pos'] <= zona_media_fin and zona_media_ini <= f2['pos'] <= zona_media_fin and jor_actual > 30:
            peso_h *= 0.93; peso_a *= 0.93
        # --- RIVALIDAD / ODIO ---
        h2h = df_r[((df_r['HomeTeam']==eq1)&(df_r['AwayTeam']==eq2))|((df_r['HomeTeam']==eq2)&(df_r['AwayTeam']==eq1))].tail(6)
        if len(h2h) >= 3:
            avg_tarjetas = (h2h['HY']+h2h['AY']+h2h['HR']+h2h['AR']).mean()
            avg_faltas = (h2h['HF']+h2h['AF']).mean()
            if avg_tarjetas >= 4.5 or avg_faltas >= 28:
                peso_h *= 1.08; peso_a *= 1.08
                f1['hf'] *= 1.25; f2['hf'] *= 1.25
                f1['hy'] *= 1.3; f2['hy'] *= 1.3
                f1['hr'] *= 1.5; f2['hr'] *= 1.5
                # en derbis suele haber menos goles
                peso_h *= 0.97; peso_a *= 0.97
                        # --- VENGANZA (paliza reciente) ---
        def factor_venganza(eq, df_eq):
            ult3 = df_eq.tail(3)
            for _, r in ult3.iterrows():
                es_local = r['HomeTeam'] == eq
                gf = r['FTHG'] if es_local else r['FTAG']
                gc = r['FTAG'] if es_local else r['FTHG']
                dif = gc - gf
                if dif >= 3:  # le metieron 3+
                    return 1.12 if not es_local else 1.07  # más rabia si fue fuera
                if dif >= 4:
                    return 1.15
            return 1.0

        veng_h = factor_venganza(eq1, m1)
        veng_a = factor_venganza(eq2, m2)
        
        peso_h *= veng_h
        peso_a *= veng_a
        
        # si hay venganza, sube intensidad
        if veng_h > 1.05:
            f1['hf'] *= 1.15; f1['hy'] *= 1.2; f1['hc'] *= 1.1
        if veng_a > 1.05:
            f2['hf'] *= 1.15; f2['hy'] *= 1.2; f2['hc'] *= 1.1
                    # --- HUNDIDO (olor a descenso) ---
        def factor_hundido(eq, df_eq, pos_actual, n_total):
            ult5 = df_eq.tail(5)
            if len(ult5) < 4: return 1.0
            
            derrotas = 0; pts = 0
            for _, r in ult5.iterrows():
                es_local = r['HomeTeam'] == eq
                ftr = r['FTR']
                if (es_local and ftr == 'A') or (not es_local and ftr == 'H'):
                    derrotas += 1
                elif ftr == 'D':
                    pts += 1
                else:
            ...
            # está hundido si: 4 derrotas en 5, o ≤2 puntos en 5, Y está en el 35% bajo
            zona_baja = pos_actual > (n_total * 0.65)
            
            if (derrotas >= 4 or pts <= 2) and zona_baja and jor_actual > 18:
                return 0.82  # -18% ataque
            elif derrotas >= 3 and zona_baja:
                return 0.90
            return 1.0

        hund_h = factor_hundido(eq1, m1, f1['pos'], n_eq)
        hund_a = factor_hundido(eq2, m2, f2['pos'], n_eq)
        
        peso_h *= hund_h
        peso_a *= hund_a
        
        # equipo hundido = más tarjetas por desesperación
        if hund_h < 0.9:
            f1['hy'] *= 1.25; f1['hr'] *= 1.4; f1['hf'] *= 1.1
            f1['def'] *= 1.12  # defiende peor
        if hund_a < 0.9:
            f2['hy'] *= 1.25; f2['hr'] *= 1.4; f2['hf'] *= 1.1
            f2['def'] *= 1.12
                    # --- VIDA O MUERTE (últimas 8 jornadas) ---
        if jor_actual > jmax_all - 8:
            def factor_vida(eq_pos, eq_pts, riv_pos, riv_pts, n_eq):
                # ¿se juega algo?
                pelea_descenso = eq_pos > n_eq - 4 and eq_pts <= 40  # zona baja
                pelea_arriba = eq_pos <= 4  # champions/ascenso
                
                # ¿rival no se juega nada? (mitad tabla, sin opciones)
                rival_medio = int(n_eq*0.35) < riv_pos < int(n_eq*0.70)
                
                if (pelea_descenso or pelea_arriba) and rival_medio:
                    return 1.18  # +18% va a muerte
                return 1.0
            
            vida_h = factor_vida(f1['pos'], pts1, f2['pos'], pts2, n_eq)
            vida_a = factor_vida(f2['pos'], pts2, f1['pos'], pts1, n_eq)
            
            peso_h *= vida_h
            peso_a *= vida_a
            
            if vida_h > 1.1:
                f1['hf'] *= 1.2; f1['hy'] *= 1.25; f1['hst'] *= 1.1
            if vida_a > 1.1:
                f2['hf'] *= 1.2; f2['hy'] *= 1.25; f2['hst'] *= 1.1
        ...
        # --- RESULTADO ESPERADO (no aleatorio) ---
        g1 = int(round(lam_h)); g2 = int(round(lam_a))
        h1 = int(round(lam_h * f1['p1'])); a1 = int(round(lam_a * f2['p1']))
        h2 = max(0, g1-h1); a2 = max(0, g2-a1)

        # stats (con pequeña variación para no clonar)
        hs1 = int(f1['hs']); hst1 = int(f1['hst']); hf1 = int(f1['hf']); hc1 = int(f1['hc']); hy1 = int(round(f1['hy'])); hr1 = int(round(f1['hr']))
        hs2 = int(f2['hs']); hst2 = int(f2['hst']); hf2 = int(f2['hf']); hc2 = int(f2['hc']); hy2 = int(round(f2['hy'])); hr2 = int(round(f2['hr']))
        ...
        html = f"""
        <div style='font-family:monospace; font-size:11px; line-height:1.15; padding:4px 6px; background:#fff; border-bottom:2px solid #000'>
        {ht_res}/{ft_res}<br>
        {ab1} {g1}-{g2} {ab2}<br>
        ...
        2p:{h2}G<br>
        2p:{a2}G<br>
        {hs1}T {hst1}TP {hf1}F {hc1}C {hy1}A {hr1}R<br>
        {hs2}T {hst2}TP {hf2}F {hc2}C {hy2}A {hr2}R
        </div>
        """
