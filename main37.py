...
def calcular_estado_jornada(df):
    ...
    # Jornada - vectorizado por liga
    df['Jornada'] = 0
    for (liga, temp), g in df.groupby(['League','Season']):
    ...
    # Puntos del partido - vectorizado
    df['HPts'] = np.select([df['FTR']=='H', df['FTR']=='D'], [3,1], 0)
    df['APts'] = np.select([df['FTR']=='A', df['FTR']=='D'], [3,1], 0)

    # Puntos previos - tu versión fast
    df['HomePtsPrev'] = 0
    df['AwayPtsPrev'] = 0
    for (l,s), g in df.groupby(['League','Season']):
        home_cum = g.groupby('HomeTeam')['HPts'].cumsum().shift(1).fillna(0)
        away_cum = g.groupby('AwayTeam')['APts'].cumsum().shift(1).fillna(0)
        df.loc[g.index, 'HomePtsPrev'] = home_cum.values
        df.loc[g.index, 'AwayPtsPrev'] = away_cum.values

    # Posiciones previas y clasificación - loop ligero (igual que antes)
    datos = []
    tablas_por_jornada = []
    for (liga, temp), grupo in df.groupby(['League','Season']):
        ...
        for idx, row in grupo.iterrows():
            ...
            datos.append({'idx': idx, 'HomePosPrev': pos[h], 'AwayPosPrev': pos[a]})
            tabla[h]['PJ'] += 1; tabla[a]['PJ'] += 1
            tabla[h]['GF'] += row['FTHG']; tabla[h]['GC'] += row['FTAG']
            tabla[a]['GF'] += row['FTAG']; tabla[a]['GC'] += row['FTHG']
            if row['FTR'] == 'H':
                tabla[h]['Pts'] += 3; tabla[h]['PG'] += 1; tabla[a]['PP'] += 1
            elif row['FTR'] == 'A':
                tabla[a]['Pts'] += 3; tabla[a]['PG'] += 1; tabla[h]['PP'] += 1
            else:
                tabla[h]['Pts'] += 1; tabla[a]['Pts'] += 1; tabla[h]['PE'] += 1; tabla[a]['PE'] += 1
    ...
    if datos:
        df_pos = pd.DataFrame(datos).set_index('idx')
        df = df.join(df_pos)
    else:
        df['HomePosPrev'] = 0
        df['AwayPosPrev'] = 0
