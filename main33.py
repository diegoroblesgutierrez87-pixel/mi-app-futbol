...
def render_tabla_equipo(df_input, equipo_ref):
    ...
    if todos_eventos and not df_tmp.empty:
        ...
        df_tmp['Goles'] = ''
    # aplica filtro de jugador si está activo
    if jugador_filtro!= "TODOS":
    ...
    html = df_tmp[['partidos']].to_html(escape=False, index=False, classes='dataframe')
    return css + html

if equipo_filtro!= "Ninguno" and equipo2_filtro!= "Ninguno":
    # MODO COMPARACIÓN: dos columnas, sin romper filtros de liga/temporada/jornada
    ...
    with col_izq:
        ...
        st.components.v1.html(render_tabla_equipo(df1, equipo_filtro), height=900, scrolling=True)
    with col_der:
        st.caption(f"Mostrando {len(df2)} partidos de {equipo2_filtro}")
        st.components.v1.html(render_tabla_equipo(df2, equipo2_filtro), height=900, scrolling=True)
else:
    # TU RENDER ORIGINAL (modo 1 equipo)
    if len(df_final) > 0:
        df_final['partidos'] = df_final.apply(lambda row: formatear_partido(row, equipo_filtro, cuota_tipo, row.get('Goles','')), axis=1)
        df_final['Tarjetas/Corners/goles'] = df_final.apply(lambda row: crear_columna_tarjetas_corners(row, equipo_filtro), axis=1)
    else:
    ...
    df_html = df_final[['partidos']].copy()
    html_table = df_html.to_html(escape=False, index=False, classes='dataframe')
    st.components.v1.html(css + html_table, height=900, scrolling=True)
